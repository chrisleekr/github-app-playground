...
- **FR-010**: The platform MUST surface the dispatch decision in the user-visible tracking comment, including chosen target and reason, and — when triage ran — confidence and rationale in a collapsible section. The `reason` value MUST be drawn from the canonical enum: `label-override` (FR-004), `keyword-match` (FR-005), `static-default` (no label/keyword match in a non-auto mode), `triage` (auto-mode triage at or above threshold), `default-fallback` (auto-mode triage below threshold), `triage-error-fallback` (auto-mode triage timed out, failed, or returned an unknown mode), `infra-absent` (FR-018 — isolated-job infrastructure missing), and `capacity-rejected` (FR-018 — pending queue at configured maximum).
