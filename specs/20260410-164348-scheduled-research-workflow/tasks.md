...
- [ ] T020 [US3] **(USER ACTION)** Push the branch with `git push -u origin 20260410-164348-scheduled-research-workflow`, then run the manual smoke test exactly as documented in `specs/20260410-164348-scheduled-research-workflow/quickstart.md` Pre-merge §4 (`gh workflow run research.yml --ref 20260410-164348-scheduled-research-workflow --field focus_area=docs`, then `gh run watch --exit-status`, then verify the outcome per §4d, then verify zero side effects per §4e). The smoke-test run URL does **not** need to be captured manually — T024 fetches it deterministically from the `gh run list` API.
...
**Checkpoint 5**: All three user stories are demonstrably working. The workflow file has been audited statically (T018), audited by pattern (T019, T021, T022), and validated end-to-end (T020). The repo is ready for the Polish phase.
...
- [ ] T024 [P] Draft the PR description body using the project's PR template at `.github/PULL_REQUEST_TEMPLATE.md`. Fetch the most recent smoke-test run URL deterministically with `gh run list --workflow research.yml --branch 20260410-164348-scheduled-research-workflow --limit 1 --json url,status,conclusion --jq '.[0]'` and confirm `status == "completed"` and `conclusion == "success"` before pasting into the PR description. Include: (a) one-paragraph summary, (b) link to `specs/20260410-164348-scheduled-research-workflow/spec.md`, (c) link to `specs/20260410-164348-scheduled-research-workflow/plan.md`, (d) the fetched smoke-test run URL, (e) explicit confirmation that the smoke test passed all six verification steps from `quickstart.md` Pre-merge §4d–§4e, (f) the Constitution Check result from `plan.md` (PASS with one Complexity Tracking entry for Principle V), (g) the explicit list of FRs satisfied (FR-001 through FR-019).
...
- [ ] T026 **(USER ACTION)** Final pre-merge sanity check before the user merges the PR: confirm the smoke test is still green (`gh run list --workflow research.yml --limit 1` shows the most recent run as `completed success`); confirm both required secrets still exist (`gh secret list --repo chrisleekr/github-app-playground` shows `CLAUDE_CODE_OAUTH_TOKEN` and `PERSONAL_ACCESS_TOKEN` with recent timestamps); confirm the workflow file is the only `.github/workflows/*.yml` file modified by this PR (`git diff --name-only main..HEAD -- .github/workflows/`); only after all three confirmations succeed, merge the PR.
...
- **User Story 3 (Phase 5, P3)**: Depends on Phases 3 + 4 being feature-complete because the smoke test (T020) exercises both schedule-trigger paths and workflow_dispatch-trigger paths. Tasks T018, T019, T021, T022 can run in parallel with each other; T020 is the long-running one and is independent of the other four (it can be started in parallel and watched in the background).
- **Polish (Phase 6)**: Depends on US3 (Phase 5) completion — specifically the smoke-test run URL from T020 is referenced in the PR description (T024).
