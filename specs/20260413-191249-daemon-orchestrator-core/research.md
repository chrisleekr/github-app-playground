...
```typescript
function selectDaemon(activeDaemons: DaemonInfo[], jobHints: JobHints): DaemonInfo | null {
...
function hasRequiredTools(caps: DaemonCapabilities, required: string[]): boolean {
  const availableToolNames = new Set(caps.cliTools.filter((t) => t.functional).map((t) => t.name));
...
switch (compat) {
  ...
  case "app-ahead":
    ...
    log.warn(
      { daemonVersion: daemon.appVersion, serverVersion: OWN_APP_VERSION },
