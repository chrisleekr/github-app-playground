...
| #     | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | File           |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| 8.F.1 | Edit `package.json` `overrides` block: remove the `^` prefix from every entry so the versions are exact pins. **Do NOT add a nested `"//"` rationale key inside `overrides`** — npm rejects unknown keys inside `overrides` with `npm error Override without name: //`, breaking any CI step that runs `npm install` (semantic-release bootstraps its own packages via npm). bun tolerates the key but npm is the strict consumer here. The rationale lives in this plan section + the commit message instead. | `package.json` |
