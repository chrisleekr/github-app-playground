...
| P8    | Scoped commands across issues + PRs: `bot:fix-thread`, `bot:explain-thread`, `bot:summarize`, `bot:rebase`, `bot:investigate`, `bot:triage`, `bot:open-pr`. NL classifier intent enum widened 5→12 with per-event-surface eligibility; label set extended. Webhook subscriptions extended to `pull_request_review_comment.created`, `issues.opened/labeled/edited`, and `issue_comment` filtered to issues. `SHIP_USE_TRIGGER_SURFACES_V2` flag deleted as P8 prerequisite (NL + label surfaces become permanent). No new database tables — scoped commands are stateless one-shots and do NOT consume a `ship_intents` session row. | FR-029, FR-030, FR-031, FR-032, FR-033, FR-034, FR-035 | Surface broadening: maintainer-scoped automation across issues + PRs without spinning up a shepherding session. Shares trigger plumbing with shepherding for code reuse; independently shippable from P1–P7. |

**Phase ordering rationale**: P1 alone fixes the immediate "premature ready" bug with one new file (probe) + one terminal-condition change. P2 is the data foundation. P3 is the architecture payload (continuation + slot release). P4 makes it reactive. P5 adds discipline. P6 adds the safety valve. P7 closes the verifiability gap. P7 could ship before P1 if the unverified-resolution gap proves more painful than the premature-ready bug — defer that ordering decision to whichever evidence surfaces first. **P8 is independently shippable from P1–P7** (no dependency on probe/intents/continuations) but reuses the trigger-router scaffolding from US1 (T014b) and the NL classifier from US1 (T014c); ship P8 after at least one of those is in main so the routing layer exists.

### P8 architecture notes (FR-029..FR-035 scoped commands)

- **One-shot lifecycle, no session**: each scoped command writes no `ship_*` rows, holds no daemon slot beyond its own short execution, and does NOT compute `MergeReadiness`. They are fire-and-forget jobs whose only durable artifact is a GitHub-side state change (a comment, a commit, a PR creation, or a label self-removal).
- **Concurrency**: any number of scoped commands may run concurrently with an active ship session on the same PR, and with each other. FR-007a's single-active-session rule does not apply to them.
- **Authorization parity**: `ALLOWED_OWNERS` (FR-028) gates every scoped command identically to ship commands. No per-command tiering.
- **Trigger plumbing reuse**: the three-surface trigger router from US1 (literal / NL / label) routes scoped commands without modification. The NL classifier intent enum widens from 5 → 12 entries; the label set widens by 7. Per-event-surface eligibility (e.g., `bot:investigate` is meaningless on PR review comments) is enforced inside the classifier and the label dispatcher — `intent: 'none'` is returned on non-matching surfaces with zero handler invocations.
- **`bot:open-pr` meta-issue gate**: before any branch/PR creation, `bot:open-pr` issues a single-turn Bedrock classification call (reusing `src/ai/llm-client.ts`) to determine whether the issue is actionable. The classifier returns `{ actionable: boolean, kind: 'bug' | 'feature' | 'tracking' | 'meta' | 'roadmap' | 'discussion' | 'unclear', reason: string }`. Non-actionable verdicts result in an issue-comment reply citing the classifier's reason and exit; the bot does not proceed to branch creation. No `--force` override in v1 — if the classifier mislabels, the maintainer manually opens the PR. Cost: one LLM call per `bot:open-pr` trigger, same envelope as FR-025.
- **`bot:triage` is suggest-only in v1**: posts a single proposal comment containing recommended labels, severity, and duplicate candidates. The bot does NOT call `addLabelsToLabelable`, `removeLabelsFromLabelable`, `closeIssue`, `lockLockable`, `addAssigneesToAssignable`, or any other state-mutating GraphQL mutation. Apply-mode is a deliberate v2 follow-up gated on a feedback-loop dataset (proposed-vs-applied agreement rate); building apply-mode in v1 is forbidden.
- **`bot:rebase` is forward-only**: uses `git merge` of base into head (or equivalent GitHub API). Force-push, history rewrite, and any `git push +<refspec>` are forbidden per FR-009. Conflicts halt the command with a maintainer-facing reply naming the conflicting paths — the bot never resolves conflicts speculatively.
- **`bot:summarize` and `bot:investigate` posts are markered**: each uses a stable HTML-comment marker (e.g., `<!-- bot:summarize:{pr_number} -->`, `<!-- bot:investigate:{issue_number} -->`) distinct from the shepherding tracking-comment marker (FR-006), so re-triggers update in place rather than duplicating.
