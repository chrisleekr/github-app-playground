...
- **`bot:summarize` and `bot:investigate` posts use markers**: each uses a stable HTML-comment marker (e.g., `<!-- bot:summarize:{pr_number} -->`, `<!-- bot:investigate:{issue_number} -->`) distinct from the shepherding tracking-comment marker (FR-006), so re-triggers update in place rather than duplicating.
