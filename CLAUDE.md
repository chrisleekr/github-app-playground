...
Three contracts contributors MUST preserve when touching the agent execution path or any GitHub-bound write:
...
3. **Input sanitization chokepoint** (`src/core/formatter.ts` + `src/utils/sanitize.ts sanitizeContent()`). Every attacker-controllable string that crosses into `buildPrompt` (PR/issue title, author login, branch names, body, comment bodies, review-comment bodies, review-comment paths, changed-file filenames) MUST pass through `sanitizeContent` at its formatter site. The pipeline strips zero-width / bidi-override / control characters, hidden HTML attributes, markdown image alt-text injection vectors, and known-format GitHub tokens — defenses load-bearing for the spotlit `<untrusted_*>` tag boundaries the `<security_directive>` block (`src/core/prompt-builder.ts`) relies on. When you add a new field to `FetchedData` or a new formatter helper, mirror the existing pattern (see `formatReviewComments` for body+path parity) — do not bare-interpolate. The `triggerBody` is sanitized via `sanitizeContent` at `src/core/prompt-builder.ts` for the same reason. Bounded GitHub-schema fields (`author.login`, ISO timestamps, `PatchStatus` enum) are exempt.
...
- 20260507-resolve-ci-recheck-gate: Closes #93 — added a fourth `HandlerResult` terminal variant `incomplete` (`{ status: "incomplete", reason, state?, humanMessage? }`) to distinguish "agent ran cleanly but work remains" from a true pipeline error, and wired the resolve handler to use it. After `runPipeline()` returns successfully, the handler now (a) re-fetches `pulls.get` to pick up any new HEAD SHA the agent pushed, (b) re-paginates `checks.listForRef` against that SHA via the new shared `evaluateChecks` helper (`src/workflows/handlers/checks.ts` — single source of truth for the prologue snapshot AND the post-pipeline re-check), (c) parses `## Outstanding` from `RESOLVE.md` via `parseOutstandingSection` (`src/workflows/handlers/resolve-report.ts`), and only returns `succeeded` when CI is all-green AND the section is empty/absent. Otherwise returns `incomplete` with the outstanding content surfaced in the tracking comment. DB migration `009_workflow_runs_incomplete.sql` extends `workflow_runs.status` CHECK to include `'incomplete'`; `runs-store.markIncomplete()` mirrors `markFailed()` with `state.incompleteReason`. The daemon executor maps `incomplete` → `failed` with reason prefixed `incomplete:` for the orchestrator's binary `succeeded | failed` cascade contract; the orchestrator detects that prefix and writes a distinct parent message ("ship halted at step N — resolve returned incomplete; see PR tracking comment") instead of the generic failure text. The "all-green" definition (no `failure`/`cancelled`/`timed_out`/`action_required` conclusions) now lives in one helper rather than duplicated in the handler prologue.
