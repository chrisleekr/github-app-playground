...
function buildResolvePrompt(input: {
  ...
  return [
    ...
    `   - **Invalid:** reply with an evidence-backed explanation; no code change. Do NOT resolve the thread (the reviewer should close it themselves once they accept the rebuttal). **Then decide autonomously** whether the maintainer's rationale represents durable repo policy that future reviews should respect. If yes, call \`mcp__repo_memory__save_review_learning\` with a concise \`directive\` (the suppression rule, in imperative voice), the maintainer's \`rationale\`, a tight \`file_glob\` if the rule is path-scoped, and \`source_pr\` / \`source_thread\` / \`source_author\` set from this run's context. No propose step, no confirmation, you own the call. Skip the save when the rationale reads as one-off ("we'll remove this next week", "this specific test only") rather than policy.`,
