---
tags:
  - contributing
---

# Contributing

The contributor guide lives at the repo root so it renders on the GitHub code view. See [CONTRIBUTING.md on GitHub](https://github.com/chrisleekr/github-app-playground/blob/main/CONTRIBUTING.md) for local setup, testing, linting, and commit message conventions.

If your PR touches code that the operator-facing docs describe, update the matching page under `docs/` in the same change — see the `## Documentation` section of [CLAUDE.md](https://github.com/chrisleekr/github-app-playground/blob/main/CLAUDE.md).
