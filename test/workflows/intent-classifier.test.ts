...
describe("intent-classifier fixture accuracy (T034)", () => {
  it("fixture set covers ≥27 comments with ≥3 per atomic (triage/plan/implement/review/resolve) / ship / clarify / unsupported / chat-thread", () => {
    // 9 keys × ≥3 each = 27 floor. The per-key loop below is the strict
    // gate; this length floor is a quick fail-fast for the common case
    // where someone removes fixtures wholesale.
    expect(fixtureSet.length).toBeGreaterThanOrEqual(27);
